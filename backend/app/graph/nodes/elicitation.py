...
class ElicitationAgent:
    def run(self, state: AgentState) -> RecommendResponse:
        ...
        debug_print("DEBUG_PIPELINE", "elicitation.input", {"turns_used": turns_used, "missing": list(state.get("missing") or []), "profile": {"budget_vnd": profile.get("budget_vnd"), "seats": profile.get("seats")}})
        print("elicitation_node")
        ...
        debug_print("DEBUG_PIPELINE", "elicitation.output", {"client_turns_used": client_state["turns_used"], "question_ids": [q.id for q in questions]})
